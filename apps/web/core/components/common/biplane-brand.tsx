...
// Biplane lockup: glyph + wordmark, drop-in replacement for PlaneLockup usages.
export function BiplaneLockup({ height = 20, className }: { height?: number; width?: number; className?: string }) {
  const size = Math.round(height * 1.5);
  return (
    <span className={"flex items-center gap-2 " + (className ?? "")} style={{ height }}>
      <BiplaneLogo size={size} />
      <span style={{ fontSize: height, lineHeight: 1 }} className="font-semibold tracking-tight">
        Biplane
      </span>
    </span>
  );
